...
// calendrier_officiel pour l'équipe Sablé-sur-Sarthe FC (National 2),
// avant d'ajouter son effectif, pour choisir un nom de club qui se
// rapprochera correctement via clubWordsMatch
...
const { data, error } = await supabase
  ...
  .eq('division', 'N2')
  .eq('saison', '2026-2027')
  .or('equipe_domicile.ilike.%sable%,equipe_exterieur.ilike.%sable%');

if (error) { console.error('Erreur lecture calendrier_officiel :', error.message); process.exit(1); }

const noms = new Set();
for (const m of data || []) {
  if (/sable/i.test(m.equipe_domicile)) noms.add(m.equipe_domicile);
  if (/sable/i.test(m.equipe_exterieur)) noms.add(m.equipe_exterieur);
}
