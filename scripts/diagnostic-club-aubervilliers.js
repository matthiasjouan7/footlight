// Diagnostic lecture seule : trouve le nom exact du club FCM Aubervilliers
...
const candidats = [...noms].filter((n) => (n || '').toLowerCase().includes('aubervilliers'));
console.log(`Candidat(s) contenant "aubervilliers" : ${candidats.length}`);
...
console.log(`\nJoueur(s) déjà en base avec club contenant "aubervilliers" : ${joueurs?.length || 0}`);
