// Diagnostic lecture seule : trouve le nom exact du club Thonon Évian
// Grand Genève FC dans calendrier_officiel (National 2, saison 2026-2027),
...
const { data: joueurs, error: errJ } = await supabase
  ...
  .or('club.ilike.%thonon%,club.ilike.%evian%,club.ilike.%évian%');
if (errJ) { console.error('Erreur lecture joueurs :', errJ.message); process.exit(1); }
console.log(`\nJoueur(s) déjà en base avec club contenant "thonon"/"evian" : ${joueurs?.length || 0}`);
