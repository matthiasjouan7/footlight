...
// calendrier_officiel pour l'équipe ASPTT Caen (National 2), avant
...
const noms = new Set();
for (const m of data || []) {
  if (/caen/i.test(m.equipe_domicile)) noms.add(m.equipe_domicile);
  if (/caen/i.test(m.equipe_exterieur)) noms.add(m.equipe_exterieur);
}
