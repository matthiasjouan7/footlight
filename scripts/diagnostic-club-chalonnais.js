// Diagnostic lecture seule : trouve le nom exact du club FC Chalonnais dans
...
const candidats = [...noms].filter((n) => (n || '').toLowerCase().includes('chalon'));
console.log(`Candidat(s) contenant "chalon" : ${candidats.length}`);
...
console.log(`\nJoueur(s) déjà en base avec club contenant "chalon" : ${joueurs?.length || 0}`);
