// Diagnostic lecture seule : trouve le nom exact du club US Ivry Football
...
const candidats = [...noms].filter((n) => (n || '').toLowerCase().includes('ivry'));
console.log(`Candidat(s) contenant "ivry" : ${candidats.length}`);
...
console.log(`\nJoueur(s) déjà en base avec club contenant "ivry" : ${joueurs?.length || 0}`);
