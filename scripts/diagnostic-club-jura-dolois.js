// Diagnostic lecture seule : trouve le nom exact du club Jura Dolois
// Football dans calendrier_officiel (National 2, saison 2026-2027), avant de
// créer le script d'ajout d'effectif.
...
const candidats = [...noms].filter((n) => (n || '').toLowerCase().includes('dolois') || (n || '').toLowerCase().includes('jura'));
console.log(`Candidat(s) contenant "dolois"/"jura" : ${candidats.length}`);
for (const c of candidats) console.log(`  "${c}"`);

const { data: joueurs, error: errJ } = await supabase
  .from('joueurs')
  .select('id, prenom, nom, club')
  .or('club.ilike.%dolois%,club.ilike.%jura%');
if (errJ) { console.error('Erreur lecture joueurs :', errJ.message); process.exit(1); }
console.log(`\nJoueur(s) déjà en base avec club contenant "dolois"/"jura" : ${joueurs?.length || 0}`);
