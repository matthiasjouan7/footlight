// Diagnostic lecture seule : trouve le nom exact du club AS Steenvoorde
...
const candidats = [...noms].filter((n) => (n || '').toLowerCase().includes('steenvoorde'));
console.log(`Candidat(s) contenant "steenvoorde" : ${candidats.length}`);
for (const c of candidats) console.log(`  "${c}"`);

const { data: joueurs, error: errJ } = await supabase
  .from('joueurs')
  .select('id, prenom, nom, club')
  .ilike('club', '%steenvoorde%');
if (errJ) { console.error('Erreur lecture joueurs :', errJ.message); process.exit(1); }
console.log(`\nJoueur(s) déjà en base avec club contenant "steenvoorde" : ${joueurs?.length || 0}`);
