// Diagnostic lecture seule : trouve le nom exact du club FC Mulhouse dans
...
const candidats = [...noms].filter((n) => (n || '').toLowerCase().includes('mulhouse'));
console.log(`Candidat(s) contenant "mulhouse" : ${candidats.length}`);
...
console.log(`\nJoueur(s) déjà en base avec club contenant "mulhouse" : ${joueurs?.length || 0}`);
