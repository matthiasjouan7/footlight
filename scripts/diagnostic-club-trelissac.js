...
// calendrier_officiel pour l'équipe Trélissac FC / APFC (National 2 groupe
...
const { data, error } = await supabase
  ...
  .eq('division', 'N2')
  .eq('saison', '2026-2027')
  .or('equipe_domicile.ilike.%trelissac%,equipe_exterieur.ilike.%trelissac%,equipe_domicile.ilike.%trélissac%,equipe_exterieur.ilike.%trélissac%');

if (error) { console.error('Erreur lecture calendrier_officiel :', error.message); process.exit(1); }

const noms = new Set();
for (const m of data || []) {
  if (/trelissac|tr[eé]lissac/i.test(m.equipe_domicile)) noms.add(m.equipe_domicile);
  if (/trelissac|tr[eé]lissac/i.test(m.equipe_exterieur)) noms.add(m.equipe_exterieur);
}
