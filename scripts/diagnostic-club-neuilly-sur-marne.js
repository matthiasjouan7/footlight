// Diagnostic lecture seule : trouve le nom exact du club SFC Neuilly-sur-Marne
...
const candidats = [...noms].filter((n) => (n || '').toLowerCase().includes('neuilly'));
console.log(`Candidat(s) contenant "neuilly" : ${candidats.length}`);
...
console.log(`\nJoueur(s) déjà en base avec club contenant "neuilly" : ${joueurs?.length || 0}`);
