// Diagnostic lecture seule : trouve le nom exact du club AS Nancy-Lorraine B
...
const candidats = [...noms].filter((n) => (n || '').toLowerCase().includes('nancy'));
console.log(`Candidat(s) contenant "nancy" : ${candidats.length}`);
...
console.log(`\nJoueur(s) déjà en base avec club contenant "nancy" : ${joueurs?.length || 0}`);
