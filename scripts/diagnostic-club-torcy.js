// Diagnostic lecture seule : trouve le nom exact du club US Torcy dans
...
const candidats = [...noms].filter((n) => (n || '').toLowerCase().includes('torcy'));
console.log(`Candidat(s) contenant "torcy" : ${candidats.length}`);
...
console.log(`\nJoueur(s) déjà en base avec club contenant "torcy" : ${joueurs?.length || 0}`);
