...
// calendrier_officiel pour l'équipe US Alençon 61 (National 2), avant
...
const { data, error } = await supabase
  ...
  .eq('division', 'N2')
  .eq('saison', '2026-2027')
  .or('equipe_domicile.ilike.%alencon%,equipe_exterieur.ilike.%alencon%');

if (error) { console.error('Erreur lecture calendrier_officiel :', error.message); process.exit(1); }

const noms = new Set();
for (const m of data || []) {
  if (/alencon/i.test(m.equipe_domicile)) noms.add(m.equipe_domicile);
  if (/alencon/i.test(m.equipe_exterieur)) noms.add(m.equipe_exterieur);
}
