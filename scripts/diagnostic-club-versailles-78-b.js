// Diagnostic lecture seule : trouve le nom exact du club FC Versailles 78 B
...
const candidats = [...noms].filter((n) => (n || '').toLowerCase().includes('versailles'));
console.log(`Candidat(s) contenant "versailles" : ${candidats.length}`);
for (const c of candidats) console.log(`  "${c}"`);

const { data: joueurs, error: errJ } = await supabase
  .from('joueurs')
  .select('id, prenom, nom, club')
  .ilike('club', '%versailles%');
if (errJ) { console.error('Erreur lecture joueurs :', errJ.message); process.exit(1); }
console.log(`\nJoueur(s) déjà en base avec club contenant "versailles" : ${joueurs?.length || 0}`);
