// Diagnostic lecture seule : trouve le nom exact du club Dijon FCO B dans
...
const candidats = [...noms].filter((n) => (n || '').toLowerCase().includes('dijon'));
console.log(`Candidat(s) contenant "dijon" : ${candidats.length}`);
...
console.log(`\nJoueur(s) déjà en base avec club contenant "dijon" : ${joueurs?.length || 0}`);
