...
// calendrier_officiel pour l'équipe SA Gazinet Cestas (National 2 groupe A),
...
const { data, error } = await supabase
  ...
  .eq('division', 'N2')
  .eq('saison', '2026-2027')
  .or('equipe_domicile.ilike.%cestas%,equipe_exterieur.ilike.%cestas%,equipe_domicile.ilike.%gazinet%,equipe_exterieur.ilike.%gazinet%');

if (error) { console.error('Erreur lecture calendrier_officiel :', error.message); process.exit(1); }

const noms = new Set();
for (const m of data || []) {
  if (/cestas|gazinet/i.test(m.equipe_domicile)) noms.add(m.equipe_domicile);
  if (/cestas|gazinet/i.test(m.equipe_exterieur)) noms.add(m.equipe_exterieur);
}
