// Diagnostic lecture seule : trouve le nom exact du club US Vimy dans
...
const candidats = [...noms].filter((n) => (n || '').toLowerCase().includes('vimy'));
console.log(`Candidat(s) contenant "vimy" : ${candidats.length}`);
...
console.log(`\nJoueur(s) déjà en base avec club contenant "vimy" : ${joueurs?.length || 0}`);
