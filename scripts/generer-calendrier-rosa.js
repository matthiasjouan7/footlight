// Rattrapage calendrier ciblé pour Ruben Rosa (QRM) uniquement — même
// modèle que generer-calendrier-bensoula.js/majid.js/gassama.js. Contrairement
// à ces trois-là, Rosa a été créé directement avec niveau="Ligue 3" (pas de
// bug de niveau à corriger), donc le script de correctif groupé (qui ne
// cible que les niveaux erronés) ne le couvre pas.
...
const { data: joueur, error: errJ } = await supabase.from('joueurs').select('id, prenom, nom, club, niveau, saison').eq('prenom', 'Ruben').eq('nom', 'Rosa').single();
...
const { data: existants, error: errE } = await supabase.from('matchs_joueur').select('calendrier_officiel_id, date_match').eq('joueur_id', joueur.id);
