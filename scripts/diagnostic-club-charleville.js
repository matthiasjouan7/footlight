// Diagnostic lecture seule : trouve le nom exact du club Olympique
// Charleville Prix Ardenne Mét. dans calendrier_officiel (National 2,
// saison 2026-2027), avant de créer le script d'ajout d'effectif.
...
const candidats = [...noms].filter((n) => (n || '').toLowerCase().includes('charleville'));
console.log(`Candidat(s) contenant "charleville" : ${candidats.length}`);
for (const c of candidats) console.log(`  "${c}"`);

const { data: joueurs, error: errJ } = await supabase
  .from('joueurs')
  .select('id, prenom, nom, club')
  .ilike('club', '%charleville%');
if (errJ) { console.error('Erreur lecture joueurs :', errJ.message); process.exit(1); }
console.log(`\nJoueur(s) déjà en base avec club contenant "charleville" : ${joueurs?.length || 0}`);
