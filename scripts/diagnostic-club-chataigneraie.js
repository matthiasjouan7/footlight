...
// calendrier_officiel pour l'équipe AS La Châtaigneraie (National 2), avant
...
const { data, error } = await supabase
  ...
  .eq('division', 'N2')
  .eq('saison', '2026-2027')
  .or('equipe_domicile.ilike.%hataigneraie%,equipe_exterieur.ilike.%hataigneraie%');

if (error) { console.error('Erreur lecture calendrier_officiel :', error.message); process.exit(1); }

const noms = new Set();
for (const m of data || []) {
  if (/hataigneraie/i.test(m.equipe_domicile)) noms.add(m.equipe_domicile);
  if (/hataigneraie/i.test(m.equipe_exterieur)) noms.add(m.equipe_exterieur);
}
