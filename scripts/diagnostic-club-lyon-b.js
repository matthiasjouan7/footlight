// Diagnostic lecture seule : trouve le nom exact du club Olympique Lyonnais B
...
const candidats = [...noms].filter((n) => (n || '').toLowerCase().includes('lyon'));
console.log(`Candidat(s) contenant "lyon" : ${candidats.length}`);
...
console.log(`\nJoueur(s) déjà en base avec club contenant "lyon" : ${joueurs?.length || 0}`);
