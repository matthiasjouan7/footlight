// Diagnostic lecture seule : trouve le nom exact du club RC Strasbourg
// Alsace B dans calendrier_officiel (National 2, saison 2026-2027), avant
// de créer le script d'ajout d'effectif.
...
const candidats = [...noms].filter((n) => (n || '').toLowerCase().includes('strasbourg'));
console.log(`Candidat(s) contenant "strasbourg" : ${candidats.length}`);
for (const c of candidats) console.log(`  "${c}"`);

const { data: joueurs, error: errJ } = await supabase
  .from('joueurs')
  .select('id, prenom, nom, club')
  .ilike('club', '%strasbourg%');
if (errJ) { console.error('Erreur lecture joueurs :', errJ.message); process.exit(1); }
console.log(`\nJoueur(s) déjà en base avec club contenant "strasbourg" : ${joueurs?.length || 0}`);
