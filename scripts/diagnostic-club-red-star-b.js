// Diagnostic lecture seule : trouve le nom exact du club Red Star FC B
...
const candidats = [...noms].filter((n) => (n || '').toLowerCase().includes('red star'));
console.log(`Candidat(s) contenant "red star" : ${candidats.length}`);
...
console.log(`\nJoueur(s) déjà en base avec club contenant "red star" : ${joueurs?.length || 0}`);
