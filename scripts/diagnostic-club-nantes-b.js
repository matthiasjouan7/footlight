...
// calendrier_officiel pour l'équipe FC Nantes B (National 2 groupe B),
...
const { data, error } = await supabase
  ...
  .eq('division', 'N2')
  .eq('saison', '2026-2027')
  .or('equipe_domicile.ilike.%nantes%,equipe_exterieur.ilike.%nantes%');

if (error) { console.error('Erreur lecture calendrier_officiel :', error.message); process.exit(1); }

const noms = new Set();
for (const m of data || []) {
  if (/nantes/i.test(m.equipe_domicile)) noms.add(m.equipe_domicile);
  if (/nantes/i.test(m.equipe_exterieur)) noms.add(m.equipe_exterieur);
}
