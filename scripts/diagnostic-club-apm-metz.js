// Diagnostic lecture seule : trouve le nom exact du club APM Metz dans
...
const candidats = [...noms].filter((n) => (n || '').toLowerCase().includes('apm'));
console.log(`Candidat(s) contenant "apm" : ${candidats.length}`);
...
console.log(`\nJoueur(s) déjà en base avec club contenant "apm" : ${joueurs?.length || 0}`);
