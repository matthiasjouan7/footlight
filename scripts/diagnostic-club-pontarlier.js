// Diagnostic lecture seule : trouve le nom exact du club CA Pontarlier dans
...
const candidats = [...noms].filter((n) => (n || '').toLowerCase().includes('pontarlier'));
console.log(`Candidat(s) contenant "pontarlier" : ${candidats.length}`);
for (const c of candidats) console.log(`  "${c}"`);

const { data: joueurs, error: errJ } = await supabase
  .from('joueurs')
  .select('id, prenom, nom, club')
  .ilike('club', '%pontarlier%');
if (errJ) { console.error('Erreur lecture joueurs :', errJ.message); process.exit(1); }
console.log(`\nJoueur(s) déjà en base avec club contenant "pontarlier" : ${joueurs?.length || 0}`);
