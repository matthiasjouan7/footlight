// Diagnostic lecture seule : trouve le nom exact du club FC Metz B dans
...
const candidats = [...noms].filter((n) => (n || '').toLowerCase().includes('metz'));
console.log(`Candidat(s) contenant "metz" : ${candidats.length}`);
...
console.log(`\nJoueur(s) déjà en base avec club contenant "metz" : ${joueurs?.length || 0}`);
