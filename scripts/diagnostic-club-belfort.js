// Diagnostic lecture seule : trouve le nom exact du club ASM Belfort dans
...
const candidats = [...noms].filter((n) => (n || '').toLowerCase().includes('belfort'));
console.log(`Candidat(s) contenant "belfort" : ${candidats.length}`);
...
console.log(`\nJoueur(s) déjà en base avec club contenant "belfort" : ${joueurs?.length || 0}`);
