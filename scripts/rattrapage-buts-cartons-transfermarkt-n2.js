// Rattrapage ciblé : sync-transfermarkt-match-stats-n2.js avait un bug qui
// empêchait TOUTE détection de but/carton/remplacement (voir son historique
// git — le type d'événement restait toujours 'inconnu' faute du bon
// sélecteur DOM, corrigé depuis). Les lignes matchs_joueur déjà écrites
// AVANT ce correctif ont donc systématiquement buts=0/cartons=0, y compris
// pour des joueurs ayant réellement marqué ou pris un carton (ex: Herman
// Lemaître, 1 but confirmé côté Transfermarkt sur son match du 29/08).
//
// Le script normal ne les retouchera JAMAIS : il saute toute ligne dont
// minutes_jouees est déjà renseigné, pour ne jamais écraser une donnée. Ce
// script fait l'inverse : il cible UNIQUEMENT les lignes déjà renseignées
// (minutes_jouees != null), reparse la même page Transfermarkt avec la
// logique corrigée, et ne met à jour que buts / cartons_jaunes /
// cartons_rouges / titulaire si la valeur recalculée diffère de celle déjà
// en base — il ne touche JAMAIS minutes_jouees (même approximatif, on ne
// veut pas risquer de dégrader une donnée déjà écrite pour un cas qu'on n'a
// pas vérifié).
//
// Sécurité : DRY_RUN=true par défaut, comme tout script d'écriture.
...
const LIMITE_MATCHS = parseInt(process.env.LIMITE_MATCHS || '30', 10);

console.log(`Mode : ${DRY_RUN ? 'DRY_RUN (aucune écriture)' : 'ÉCRITURE RÉELLE'} — Rattrapage buts/cartons National 2 groupe ${GROUPE} (wettbewerb=${WETTBEWERB}), ${NB_JOURNEES} journée(s), limite ${LIMITE_MATCHS} match(s).\n`);

// ---- Rapprochement club (identique à sync-transfermarkt-match-stats-n2.js) ----
...
// ---- 2. Pour chaque match : charge la page, extrait titre/date ----
...
// ---- 4. Ne garde que les matchs DÉJÀ (au moins partiellement) synchronisés ----
// Contrairement au script normal (qui cible les lignes encore vides), ce
// rattrapage cible les lignes DÉJÀ écrites (minutes_jouees renseigné) —
// c'est justement celles que le script normal ne retouchera plus jamais.
...
for (let i = 0; i < idsCandidats.length; i += 100) {
  ...
  const { data, error } = await supabase.from('matchs_joueur').select('id, joueur_id, calendrier_officiel_id, minutes_jouees, titulaire, buts, cartons_jaunes, cartons_rouges').in('calendrier_officiel_id', lot);
...
const matchsARattraper = matchsARapprocher.filter((m) => {
  const lignes = parCalendrierId.get(m.calendrierOfficielId) || [];
  return lignes.some((l) => l.minutes_jouees != null);
}).slice(0, LIMITE_MATCHS);
console.log(`${matchsARattraper.length} match(s) déjà synchronisé(s) à vérifier ce run (limite ${LIMITE_MATCHS}).\n`);

// ---- 5. Parseur page match (identique à sync-transfermarkt-match-stats-n2.js, avec le correctif) ----
async function parserPageMatchTransfermarkt(pageMatch) {
  ...
  return { compositionsToutes, evenements, timeline };
}
...
// ---- 6. Pour chaque match déjà synchronisé : reparse, compare, corrige buts/cartons/titulaire uniquement ----
let totalCorrections = 0, totalAmbigus = 0, totalInchanges = 0;
for (const m of matchsARattraper) {
  console.log(`--- ${m.dateTm} — ${m.equipeDomicileCal} vs ${m.equipeExterieurCal} (calendrier_officiel_id=${m.calendrierOfficielId}) ---`);
  const lignesMj = (parCalendrierId.get(m.calendrierOfficielId) || []).filter((l) => l.minutes_jouees != null);
  ...
  for (const r of resultatsParseur) {
    ...
    if (correspondances.length === 0) continue;
    if (correspondances.length > 1) {
      console.log(`  Ambiguïté : "${r.nomAffiche}" correspond à ${correspondances.length} joueurs FootLight, ignoré.`);
      totalAmbigus++;
      continue;
    }
    ...
    const changeButs = (ligneMj.buts || 0) !== r.buts;
    const changeJaunes = (ligneMj.cartons_jaunes || 0) !== r.cartonsJaunes;
    const changeRouges = (ligneMj.cartons_rouges || 0) !== r.cartonsRouges;
    const changeTitulaire = ligneMj.titulaire !== r.titulaire;
    if (!changeButs && !changeJaunes && !changeRouges && !changeTitulaire) { totalInchanges++; continue; }

    console.log(`  ${DRY_RUN ? 'À corriger' : 'Correction'} : ${joueur.prenom} ${joueur.nom} (TM: ${r.nomAffiche}) — buts ${ligneMj.buts || 0}→${r.buts}, jaunes ${ligneMj.cartons_jaunes || 0}→${r.cartonsJaunes}, rouges ${ligneMj.cartons_rouges || 0}→${r.cartonsRouges}, titulaire ${ligneMj.titulaire}→${r.titulaire} (minutes_jouees non modifié : ${ligneMj.minutes_jouees})`);
    totalCorrections++;
    if (!DRY_RUN) {
      const { error: errUpd } = await supabase.from('matchs_joueur').update({
        titulaire: r.titulaire, buts: r.buts, cartons_jaunes: r.cartonsJaunes, cartons_rouges: r.cartonsRouges,
      }).eq('id', ligneMj.id);
...
console.log(`\n========== Résumé rattrapage buts/cartons (National 2 groupe ${GROUPE}, source Transfermarkt) ==========`);
console.log(`${matchsARattraper.length} match(s) déjà synchronisé(s) vérifié(s), ${totalCorrections} correction(s) ${DRY_RUN ? 'proposée(s)' : 'effectuée(s)'}, ${totalInchanges} ligne(s) déjà correcte(s), ${totalAmbigus} ambiguïté(s) ignorée(s).`);
console.log('Note : minutes_jouees n\'est jamais modifié par ce script (uniquement buts/cartons_jaunes/cartons_rouges/titulaire).');
